...
try {
  // 1️⃣ Fetch unread messages for this user
  const messages = await Message.find({
    receiverId: userId,
    read: false,
    delivered: { $ne: true }, // ✅ Only send once
  })
    .sort({ createdAt: 1 })
  ...
  for (const msg of messages) {
    ...
    // 🔹 Convert to plain object + ISO dates
    const safeMsg = {
      ...msg._doc,
    ...
    // 3️⃣ Emit only to current socket
    socket.emit("newMessage", { message: safeMsg, conversation });

    // 4️⃣ Mark as delivered to prevent duplicate emit
    msg.delivered = true;
    await msg.save();
  }
} catch (err) {
