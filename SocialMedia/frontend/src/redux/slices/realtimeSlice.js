...
const realtimeSlice = createSlice({
  ...
  reducers: {
    ...
    setTyping: (state, action) => {
      ...
      if (isTyping) {
        state.typingUsers[conversationId][senderId] = true;
      } else {
        delete state.typingUsers[conversationId][senderId];
      }

      if (
        state.typingUsers[conversationId] &&
        Object.keys(state.typingUsers[conversationId]).length === 0
      ) {
    ...
    messageRead: (state, action) => {
      ...
      const prevConversation = state.readByConversation[conversationId] || {};
      const prevReaders = prevConversation[messageId] || [];

      if (!prevReaders.includes(readerId)) {
        state.readByConversation = {
          ...state.readByConversation,
          [conversationId]: {
            ...prevConversation,
            [messageId]: [...prevReaders, readerId],
          },
        };
      }
    },
